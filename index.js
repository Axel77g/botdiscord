...
function playsong(data){
  ...
  idconnexion.playStream(stream, streamOptions,function(suc, err){
    if(suc){
      console.log('STREAM LANCE')
    }
    if(err){
      console.error(err)
    }
...
bot.on('message', message => {
  ...
  if(message.content[0] === PREFIX){
  ...
  else if(splitmessage[0] === '!play' && splitmessage.length === 3){

    var url = splitmessage[1];
    var volume = parseInt(splitmessage[2])/100;
    play(url, volume, message)
    
 
  }else if(splitmessage[0] === '!add' && splitmessage.length === 3){
    ...
    if(fda.length >= 1){
      ...
      var delay = parseInt(lecture[1])*1000+100;
      console.log(delay);
      getinfo(url, volume, message);
    ...
    if(message.member.voiceChannel){
      ...
      idconnexion = {statut: 'none'}
      message.member.voiceChannel.leave()

    }
    message.channel.send('**:mute: DECONEXION REUSSI**')
    ...
    if(fda.length > 1){
      ...
      message.channel.send('**MORCEAU SUIVANT :headphones:**')  

    }else{
      message.channel.send('**AUCUNE AUTRE MUSIQUE DANS LA PLAYLIST**')
    }
